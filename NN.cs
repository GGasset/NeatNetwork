...
namespace NeatNetwork
{
    public class NN
    {
        ...
        public override string ToString()
        {
            ...
            List<List<Task<string>>> toStringTasks = new List<List<Task<string>>>();
            foreach (var layer in Neurons)
            {
                var layerStringTasks = new List<Task<string>>();
                foreach (var neuron in layer)
                {
                    layerStringTasks.Add(neuron.ToStringAsync());
                }

                toStringTasks.Add(layerStringTasks);
            }

            foreach (var taskLayer in toStringTasks)
            {
                foreach (var toStringTask in taskLayer)
                {
                    toStringTask.Wait();
                    str += toStringTask.Result + "_";
                }
                str += "\n";
            }
        ...
        public NN(StreamReader file, bool closeFile = true)
        {
            string currentLine;
            string currentSeparator = "HIHI";
            ...
            if (file.ReadLine() != currentSeparator)
                throw new FormatException("This file wasn't generated by NeatNetwork.");
            MaxMutationGrid = new List<List<double>>();
